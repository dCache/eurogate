...
int loop(int argc, char **argv) {
  ...
  for(;;) {
    errString[0] = '\0';
    wl[0].size = fsize;
    ret = ES_WriteData(s, wl, 1, myWcallback, NULL, NULL, 600, errString);
    if (ret != ES_OK) {
      printf("WRITE FAILED (%d) !!!! '%s' (%s)\n", ret, errString,
	     wl[0].errString);
      return(0);
    }
    printf("########### WRITE DONE ##################\n");
    strcpy(rl[0].bfid, lbfid);
    if ((ret = ES_ReadData(s, rl, 1, myRcallback, NULL, NULL, 600,
			   errString)) != ES_OK) {
      printf("READ FAILED (%d) !!!! '%s' (%s)\n", ret, errString,
	     rl[0].errString);
      return(0);
    }
    printf("$$$$$$$$$$$ READ DONE $$$$$$$$$$$$$$$$$$\n");
    strncpy(reml[0].bfid, lbfid, ES_BFID_STRSIZE);
    if ((ret = ES_Remove(s, reml, 1, myRemcallback, NULL,
			 errString)) != ES_OK) {
      printf("REMOVE FAILED (%d) !!!! '%s' (%s)\n", ret, errString,
	     reml[0].errString);
      return(0);
    }
    printf(" %%%%%%%%%%%%%%%% REMOVE DONE %%%%%%%%%%%%%%%%%%%%\n");
...
main(int argc, char **argv) {
  ...
  if (argc >= 3) {
     if (strcmp(argv[1], "read") == 0) {
       (void) readOp(argc - 1, &argv[1]);
     } else if (strcmp(argv[1], "write") == 0) {
       (void) writeOp(argc - 1, &argv[1]);
     } else {
       fprintf(stderr, "unknown cmdline command %s\n", argv[1]);
       exit(1);
     }
     exit(0);
  }

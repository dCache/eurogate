...
##################################################################
#
#    create Objectivity federated database
#
createOODB() {
#
  checkVar storeDatabase || exit $?
#
  echo "Trying to create new Objectivity/DB for Store"
#

  /usr/object/bin/oonewfd -fdnumber 666 \
                          -pagesize 4096 \
                          -lockserverhost `uname -n` \
                          -fdfilepath ${storeDatabase}/EuroGate.FDB \
                          ${storeDatabase}/EuroGate

  if [ $? -ne 0 ]; then
    echo "Creating Objectivity/DB failed"
    exit 3
  fi
  return 0
}
##################################################################
#
#    delete Objectivity federated database
#
deleteOODB() {
#
  checkVar storeDatabase || exit $?
#
  echo "Trying to delete Objectivity/DB for Store"
#
  /usr/object/bin/oodeletefd ${storeDatabase}/EuroGate

  if [ $? -ne 0 ]; then
    echo "Deleteing Objectivity/DB failed"
    return  3
  fi
  return 0
}
##################################################################
#
#       initialize the Objectivity/DB Store database
#
initOODB() {
   ...
   $JAVA eurogate.store.objectivity52.ooCreateDB ${storeDatabase}
   if [ $? -ne 0 ] ; then
       echo "Init of Objectivity/DB for Store failed" 1>&2
       return 4
   fi
   exit 0
...
##################################################################
#
#    clear Objectivity federated database
#
clearOODB() {
#
  checkVar storeDatabase || exit $?
#
  echo "Trying to clear locks/transactions on Objectivity/DB for Store"
#
  /usr/object/bin/oocleanup -local ${storeDatabase}/EuroGate

  if [ $? -ne 0 ]; then
    echo "Clearing Objectivity/DB failed"
    return 3
  fi
  return 0
}
